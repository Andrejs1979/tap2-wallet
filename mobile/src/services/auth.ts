...
class AuthService {
  ...
  async login(email: string, password: string): Promise<AuthUser> {
    try {
      ...
      await Promise.all([
        ...
        SecureStore.setItemAsync('token_expires_at', expiresAt.toString()),
      ]);
  ...
  async logout(): Promise<void> {
    try {
      ...
      if (backendLogoutFailed) {
        // Return a flag indicating backend logout failed
        // The caller can decide whether to notify the user
  ...
  private async performTokenRefresh(): Promise<AuthTokens | null> {
    ...
    if (this.refreshFailureCount > 0) {
      console.log(`Retrying token refresh after ${backoffDelay}ms (attempt ${this.refreshFailureCount + 1})`);
